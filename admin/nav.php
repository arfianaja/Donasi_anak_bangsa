    ...
    <div class="row">
      <div class="col s12 m3">
          <ul id="slide-out" class="sidenav sidenav-fixed">
              ...
              <li><a href="index.php?p=registrasi"><i class="material-icons">featured_play_list</i>Bencana</a></li>
	...
	<?php 
		...
		elseif(@$_GET['p']=="pengaduan_hapus"){
			...
			if($data['status']=="proses"){
				$delete=mysqli_query($conn,"DELETE FROM pengaduan WHERE id_pengaduan='".$_GET['id_pengaduan']."'");
				header('location:index.php?p=pengaduan');
			}
			elseif($data['status']=="selesai"){
				$delete=mysqli_query($conn,"DELETE FROM pengaduan WHERE id_pengaduan='".$_GET['id_pengaduan']."'");
				if($delete){
					$delete2=mysqli_query($conn,"DELETE FROM tanggapan WHERE id_pengaduan='".$_GET['id_pengaduan']."'");
					header('location:index.php?p=pengaduan');
				}	
			}
		...
		elseif(@$_GET['p']=="tanggapan_hapus"){
			...
			$query = mysqli_query($conn,"DELETE FROM tanggapan WHERE id_tanggapan='".$_GET['id_tanggapan']."'");
			if($query){
		...
		elseif(@$_GET['p']=="user_hapus"){
			$query = mysqli_query($conn,"DELETE FROM petugas WHERE id_petugas='".$_GET['id_petugas']."'");
			if($query){
